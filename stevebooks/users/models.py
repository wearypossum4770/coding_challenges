...
class Profile(Model):
    ...
    qualifying_children = ManyToManyField(User, related_name="qualifying_children")
    dependents = ManyToManyField(User, related_name="dependents")
    date_of_birth = DateField(blank=True, null=True)
    retention_only = BooleanField(default=False)
    do_not_contact = BooleanField(default=False)
    social_security_number = CharField(max_length=9, null=True, blank=True)
